"""Source code: https://github.com/semihorhan/semseg-outdoor-pano"""
...
class DeformConv2d_plus_Offset(nn.Module):
    """
    See deform_conv2d
    """
    ...
    def __init__(
        ...
        super(DeformConv2d_plus_Offset, self).__init__()
    ...
    def forward(self, input: Tensor, offset: Tensor = None, mask: Tensor = None) -> Tensor:
        """
        Args:
            input (Tensor[batch_size, in_channels, in_height, in_width]): input tensor
        ...
        if self.offset_input is not None:
            return deform_conv2d(input, self.offset_input.to(input.device), self.weight, self.bias, stride=self.stride,
                                padding=self.padding, dilation=self.dilation)
        else:
